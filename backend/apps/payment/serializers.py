...
class PaymentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'amount', 'currency', 'status', 'payment_method', 'decription',
            'is_successful', 'is_pending', 'can_be_refunded', 'created_at',
            'upated_at', 'processed_at'
        ]
        read_only_fields = [
            "id",
            "user",
...
class PaymentCreateSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if pending_payments:
            raise serializers.ValidationError({
                'non_field_errors': ['User has pending payments']
            })
...
class RefundCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_amount(self, value):
        if value <= 0:
            raise serializers.ValidationError('Refund amount most be positive')
        return value
