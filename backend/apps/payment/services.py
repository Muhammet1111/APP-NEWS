...
class StripeService:
        ...
        try:
            ...
            session = stripe.checkout.Session.create(
                ...
                metadata={
                    ...
                    'subscription_id': payment.subscription.id if payment.subscription else ''
                }
            )
    ...
    @staticmethod
    def refund_payment(payment: Payment, reason: str, amount: Optional[Decimal] = None ):
        try:
...
class PaymentService:
    ...
    def create_subscription_payment(user, plan: SubscriptionPlan) -> Tuple[Payment, Subscription]:
        ...
        SubscriptionHistory.objects.create(
            subscription=subscription,
            action='created',
            description=f'Subcsription created for plan {plan.name}'
        )
...
class WebhookService:
    ...
    def process_stripe_webhook(event_data:Dict) -> bool:
        try:
            ...
            if success:
                ...
                webhook_event.mark_as_filed('Processing failed')
    ...
    def _handle_payment_succeeded(event_data: Dict) -> bool:
        try:
            ...
            if not payment_id:
                logging.warning('No payment_id in payment intent metadata')
                return False
    ...
    def _handle_payment_failed(event_data: Dict) -> bool:
        try:
            ...
            if not payment_id:
                logger.warning('No payment_id in payment_intent metadata')
                return False
            ...
            logger.error(f'Error hadnling payment failed: {e}')
            return False
