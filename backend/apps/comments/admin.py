...
class CommentAdmin(admin.ModelAdmin):
    list_display = ('id', 'post_title', 'author', 'parent', 
                    'content_preview', 'parent_comment', 'is_active', 'created_at')
    ...
    def make_active(self, request, queryset):
        ...
        self.message_user(request, f"{updated} comment(s) marked as active.")
    make_active.short_description = "Mark selected comments as active"
    
    def make_inactive(self, request, queryset):
        updated = queryset.update(is_active=False)
        self.message_user(request, f"{updated} comment(s) marked as inactive.")
